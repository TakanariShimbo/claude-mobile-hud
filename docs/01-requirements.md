...
会話画面 (FR-GL-30〜33) の装飾レイヤとして、状態連動の HUD マスコット (キャラクター名 **Konoha**) を表示する。インタラクションは奪わない。

| ID | 要件 | 優先度 |
|---|---|---|
| FR-GL-80 | 会話画面の右下に HUD マスコット (Konoha) を表示する | Should |
| FR-GL-81 | ConversationMode (IDLE / LISTENING / CONFIRMING / PERMISSION_CONFIRMING) に対応するベースアニメーション状態を切り替えて表示する | Should |
| FR-GL-82 | 送信確定 (Glass 上での SEND verdict) および新規 INCOMING メッセージ到着時に、約 2 秒だけ走るアニメーション (送信=右、着信=左) をベース状態の上にオーバーレイする。画面再 mount 直後に既存履歴の最新 INCOMING で trigger しないこと | Should |
| FR-GL-83 | マスコットは履歴 / 入力欄 / Confirming / Permission の各 UI 機能を阻害しない (重なっても操作可能、ジェスチャを奪わない) | Must |
...
### Rev 6 (2026-05-19): Glass HUD マスコット (Konoha) を追加

会話画面の UX 装飾レイヤとして state-aware なマスコットを新規追加:

- **FR-GL-80〜83** 追加 (§5.2 新規サブセクション「HUD マスコット (会話画面)」)
- FR-GL-83 (UI 阻害禁止) のみ Must、他 3 件は装飾のため Should
- 詳細設計は `docs/03` §4.4 (Rev 4 追記: `sendEvent` seam) / §4.8.9 / §4.11.7
- アセット (`konoha_spritesheet.webp` 192×208 cell × 8×9 grid) は姉妹プロジェクト `AndroidStudioProjects/MyPets` の AI 生成パイプライン由来、webp としてリポジトリ内に取り込み (in-tree)
