...
- **作成日**: 2026-05-16
- **改訂**: Rev 4 (Phase 2 設計時に発覚した Hub 責務漏れを補完)
- **対象**: `claude-mobile-hud` v1.0 (POC `~/claude-channel` の再構築初版)
- **記述方針**:
...
- 著者は POC (`~/claude-channel`) で「phone + Rokid Glass で Claude Code を遠隔操作する」体験を実装し、有用性を確認した
- POC は思いつき継ぎ足しで肥大化し、wire protocol 二重定義 / state race / 責務混在などの構造的負債が蓄積した (詳細: `~/claude-channel/docs/TODO.md`)
- 本プロジェクトでは **POC の体験を維持しつつ、設計を再構築** する
...
| **Glass** | Rokid Glass (HUD ウェアラブル) 上のコンパニオンアプリ |
...
| **HUD** | Head-Up Display。Rokid Glass の透過表示部分 |
| **FGS** | Foreground Service (Android)。バックグラウンドでも継続実行するための仕組み |
| **POC** | このプロジェクトの前身 (`~/claude-channel`) |
...
- Rokid Glass は所有していてもいなくても良い (glass は optional)
...
1. ユーザは Rokid Glass を装着して別作業をしている
...
| FR-HU-14 | Phone との SSE 接続成立直後、`session_snapshot` の送出に続けて、その時点での outstanding permission を `permission` イベントとして全て再 push する。Phone 側は受信側で冪等処理 (FR-PH-46) | Must |
...
| NFR-32 | Rokid Glass + Hi Rokid アプリの最新版で動作 | Must |
...
| OOS-08 | 画像以外の添付 (PDF / 動画 / 任意ファイル) および複数添付 | 1 メッセージ 1 画像で十分 (POC 経験) |
...
- Rokid Glass + Hi Rokid 認可済み (Glass 使う場合)
...
| AC-05 (POC 課題解消) | TODO.md P1-1〜P1-7 が新要件で被覆されていることを §13 のトレース表で確認 | 全ての P1 項目に対応する FR / NFR が紐付き、Phase 5 で実装確認できる |
...
- **NFR-40a / 40b の電池上限値**: 仮置き 10% / 25%。実機計測 (POC で測定可能) 後に確定
- **NFR-41 の履歴サイズ上限**: 仮置き 20 MB/session。実機運用での感覚値で見直す
- **POC 履歴 JSON の移行可否**: 互換性要件として「移行する / しない」を v1.0 開発開始前に確定
...
- テストの粒度 / カバレッジ目標
- FGS のオーケストレータ層の物理表現 (TODO P1-5 対応の具体実装)
...
POC コードと既存 docs から抽出して機能要件 / 非機能要件 / スコープ外を起こす。

---

## 13. トレーサビリティ — POC TODO.md P1 ↔ 要件 ID

POC レビューで挙げた P1 (構造的改善) 課題が、本要件書のどこで被覆されるか。Phase 5 で AC-05 の合否判定に使う。

| POC TODO | 内容 | 対応要件 |
|---|---|---|
| P1-1 | wire protocol 共有モジュール化 | NFR-50 (drift コンパイル / CI 検出可能) |
| P1-2 | `WireEventDecoder` + `sealed BridgeEvent` | NFR-51 (decode は副作用と分離テスト可能) |
| P1-3 | JSON encode / decode を `kotlinx.serialization` 統一 | NFR-50 (定義の単一真実源) |
| P1-4 | `current_mode` + `pending_permission` race 解消 | NFR-13 (atomicity), AC-09 |
| P1-5 | `GlassConnectionService` から `MicForegroundService` 知識を分離 | NFR-52 (各 FGS の責務境界) |
| P1-6 | `MainScreen` 肥大化 + pending フィルタを Repository に集約 | NFR-51 / NFR-52 (UI と状態管理の分離。実装レベルなので Phase 3 でも追跡可) |
| P1-7 | `ChannelRepository.send()` 分解 | NFR-51 (純関数化 / テスト容易性) |

P1 以外の項目 (P2, P3) は実装フェーズ (Phase 4) で個別に取り込む。要件レベルでの紐付けは必須ではない。
