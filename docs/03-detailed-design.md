...
POC `claude-channel/rgapp/glass/.../GlassBridge.kt` は最初から `readString(args, FIELD_EVENT)`
で args (Caps) を直接読んでいた経路に近く、Phase 4 で「`:protocol` の bytes seam に揃える」
リファクタを入れた際にこの非対称を見落として全 event を silent drop していた (Phase 4
の commit `576f597` で修正)。
...
|---|---|---|---|
| §1 AC-05 POC TODO トレース | P1-1〜P1-7 全 7 件 OK + reviewer 二次レビューで Partial 3 件を消化 | ✅ PASS | `617d68e` / `7f54b1d` |
...
残作業 (Phase 5 後半):
- **#182 VerdictDispatchService cold-start 実機測定**: `tools/measure_verdict_latency.py` の `verdict_dispatch_started` event を VerdictDispatchService が出す pathway を確認した後、kill 状態シナリオを実機で実走 (force-stop で通知消失 / am kill で FGS 拒否 のため特殊実機 setup が要る)
- **AC-04 SSE 再接続シナリオ実走**: `tools/measure_reconnect_latency.py` を使い ≥10 回切断 → 中央値 < 30s を実機で確認
- **NFR-40 電池 SLO 計測**: Battery Historian + 10h 連続実機テスト
- **§6 拡張実装**: Bridge auto-reconnect / Hub TLS 終端 / release variant / Hi Rokid 認可 / Settings input validation

#### 10.4.4 AC-05 (POC TODO P1-1〜P1-7) 合否

reviewer 二次レビュー (2026-05-17) 経由で P1-1 / P1-6 / P1-7 を Partial →
OK に持ち上げ、AC-05 **PASS**:

| POC TODO | 解消 commit / コード | 状態 |
|---|---|---|
| P1-1 (wire 共有モジュール + drift CI) | `protocol/`, `bridge/test/wire-golden-smoke.test.ts` (smoke、本格は Phase 5 §7.3) | ✅ Phase 4 で smoke、本格は Phase 5 |
| P1-2 (sealed WireEvent + decoder 分離) | `protocol/.../WireEvent.kt`, `JsonCodec.kt`, `CapsCodec.kt` (pure decode) | ✅ |
| P1-3 (kotlinx.serialization 統一) | Phone/Glass の全 JSON 経路で使用、手書き parse なし | ✅ |
| P1-4 (mode + pending race 解消、atomicity) | `CurrentStateDraft` 単一 source + seq を collect 内で採番 | ✅ |
| P1-5 (Glass / Mic FGS 責務分離) | `AppLifecycleController` 集約、`GlassConnectionService` から `MicForegroundService` 参照無し | ✅ |
| P1-6 (MainScreen 分解 + pending filter Repository 集約) | `MainScreen.kt` 分解、`PhoneUiState.pendingForCurrent` (commit `617d68e`) | ✅ |
| P1-7 (`ChannelRepository.send()` 分解) | `encodeImageBase64` / `onSendSuccess` 抽出 (commit `617d68e`)、`send()` ~49 行 | ✅ |
