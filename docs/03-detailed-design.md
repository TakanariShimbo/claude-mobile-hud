...
- **作成日**: 2026-05-16
- **改訂**: Rev 4 (Glass HUD マスコット Konoha を §4.4 / §4.8.9 / §4.11.7 に反映)
- **依存**: Phase 1 (`01-requirements.md` Rev 6), Phase 2 (`02-architecture.md` Rev 4)
...
**Rev 4 追記: `sendEvent` seam (FR-GL-82)** — SEND verdict 確定時に 1 回 emit する `SharedFlow<Unit>` (`extraBufferCapacity = 4`)。Konoha overlay (§4.8.9) の右向き走り演出を `ConversationScreen` 側で trigger するための **UI-only seam** で、wire には流さない。`handleConfirming` の `SendChoice.SEND` 分岐で `_sendEvent.tryEmit(Unit)` → `bridge.sendGesture(SWIPE_FORWARD)` の順に発火する (trigger を wire 送信前に確定させるため。tryEmit が落ちても wire send には影響しないが、buffer 4 のため実用上落ちない)。
...
#### 4.8.9 `KonohaSprite` overlay 統合 (Rev 4 追加、FR-GL-80〜83)

会話画面の HUD マスコット (§4.11.7) を `holder.state` に紐付けて描画する。base 状態は `state.toKonohaState()` で 4 種にマップし、その上に 2 種のイベント駆動 overlay (`RunningRight` / `RunningLeft`) を `KONOHA_OVERLAY_MS = 2000L` だけ重ねる。

| base = `ConversationStateHolder.State` | `KonohaState` (base) |
|---|---|
| `Idle` | `Idle` |
| `Listening` | `Running` |
| `Confirming` | `Review` |
| `PermissionConfirming` | `Waiting` |

`RunningRight` / `RunningLeft` は base mapping からは到達不能 (`toKonohaState()` はベース 4 種のみ返す) — overlay 経路でのみ表示される。
...
LaunchedEffect(holder) {
    holder.sendEvent.collect {
        ...
        delay(KONOHA_OVERLAY_MS); konohaOverlay = null
    }
}
...
- **送信検出 (FR-GL-82 右)**: `holder.sendEvent` (§4.4) を `LaunchedEffect(holder)` で collect。`.collect { ... }` は逐次処理なので、`KONOHA_OVERLAY_MS` 内に SEND を連射した emission は前ハンドラの `delay` 完了まで suspend される (= overlay 表示時間が短縮されることはなく、各回 2 秒走る)
- **着信検出 (FR-GL-82 左)**: `messages` の `INCOMING` 最新 id 変化で `RunningLeft` overlay。streaming reply の text 伸長は **同一 id** なので発火しない (`ChatMessagePayload.id` は autoinc Long、§2.2.4)。`initialIncomingScan` フラグで画面再 mount 直後の履歴ロードを trigger 対象外にする (FR-GL-82 後半文)。`LaunchedEffect(messages)` は messages emit 毎に rekey するが、guard ロジックは `remember` State (`lastSeenIncomingId` / `initialIncomingScan`) で正しく動作する
- **重畳ルール**: 送信 → 着信 (またはその逆) が `KONOHA_OVERLAY_MS` 内に連続した場合、後勝ち (最後に set した overlay が表示される)。先行 effect の `delay` も並列で走り、clear 操作は冪等 (set-to-null) のため整合する。ただし先行の `delay` が後発 overlay 開始後に完了して clear した場合、後発の表示時間が縮む副作用がある (最悪ケース数十 ms オーダ)。Should 要件 (FR-GL-82) のため許容
- **描画位置 (FR-GL-83)**: ChatFrame (§4.11.5) の **外側 Box** 内で `Modifier.align(BottomEnd).offset(x = 12.dp, y = 12.dp)`。ChatFrame の `clip(shape)` の外側に張り出すため、ChatFrame の **兄弟**として描画する。`GestureBus` 経由のジェスチャ層は KonohaSprite を通らないので、表示が重なっても操作を奪わない
- **NFR-13 (mode + payload atomicity)**: Konoha overlay は **演出専用の presentation overlay** で mode/payload に feedback しない。base `state.toKonohaState()` は同じ `state: StateFlow` から派生するため NFR-13 の射程内 (1 描画フレーム原子性は base 側で保証)。overlay の独立 swap は NFR-13 対象外
...
#### 4.11.7 `KonohaSprite` (HUD マスコット、Rev 4 追加、FR-GL-80〜81)

会話画面に表示する HUD マスコット (キャラクター名 **Konoha**)。192×208 px / 8 列 × 9 行の spritesheet atlas (`drawable-nodpi/konoha_spritesheet.webp`、964 KB) を Compose `Canvas` + `drawImage(srcOffset, srcSize)` で再生する。姉妹プロジェクト `AndroidStudioProjects/MyPets` の AI 生成パイプラインで作った atlas を webp 変換して取り込み (in-tree)。

```kotlin
enum class KonohaState(val row: Int, val durationsMs: IntArray) {
    Idle         (row = 0, 6 frames × 660〜1920ms (long blink)),
    RunningRight (row = 1, 8 frames × ~120ms),
    RunningLeft  (row = 2, 8 frames × ~120ms),
    Waiting      (row = 6, 6 frames × ~150ms),
    Running      (row = 7, 6 frames × ~120ms),
    Review       (row = 8, 6 frames × ~150ms),
}
```

- atlas は 8×9 (= 72 cell) を確保しているが **使うのは 6 行のみ** (rows 0,1,2,6,7,8)。rows 3-5 (Waving / Jumping / Failed) は `MyPets` 側 `PetSprite.kt` には存在するが Glass では参照しない
- frame 数は state ごとに 6 または 8 で **一定でない**。`KonohaState.frameCount` は `durationsMs.size` から導出する
- frame 進行: `LaunchedEffect(state)` 内の `while(true) { delay(durationsMs[frame].toLong()); frame = (frame + 1) % frameCount }` ループ。`remember(state)` で frame=0 リセットするので state 切り替え時に前 state の途中フレームを引き継がない
- 描画スケール `scale: Float = 0.23f` (44 × 48 dp 相当)。Rokid HUD で 12sp 文字と並んで邪魔にならず、状態差分が読める下限
- `FilterQuality.None` で pixel-art の階段感を保つ (Rokid HUD はアンチエイリアスが眩しい)
- atlas は `ImageBitmap.imageResource(R.drawable.konoha_spritesheet)` で都度 load。Compose の resource cache に乗るため実コストは初回のみ
- `ConversationStateHolder.State` → `KonohaState` の base mapping (§4.8.9 のテーブル参照) は同ファイルに `internal fun toKonohaState()` extension で集約

**asset の同期方針**: `MyPets/app/src/main/res/drawable-nodpi/konoha_spritesheet.webp` を真として参照。グリッド構造 (192×208、8×9) と `durationsMs` 配列は MyPets 側 `PetSprite.kt` の Konoha 行に揃える (atlas 更新時は両側を更新)。
...
### 8.6 Android (Kotlin) `StructuredLog` 実装詳細

`phone/log/StructuredLog.kt` と `glass/log/StructuredLog.kt` は両 app それぞれが
持つ Logcat wrapper。§8.3 / §8.4 の key=value 形式は TS 側 (§8.5) と grammar parity
を保ち、`tools/verify_atomicity.py` の parser が両方を同 grammar で扱える契約。

#### 8.6.1 JVM unit-test seam (P3-8)

`companion object` の `internal fun formatLine(event, fields): String` と
`internal fun formatValue(v): String` を JVM 直接呼び出し可能に公開する。Android
`Log.i()` を介さず純粋関数で escape 規則を検証する目的 (`androidTest` ではなく
`test` で走らせる)。Robolectric / MockK 依存を引き込まず、`quote()` / `NEEDS_QUOTE` /
`CONTROL_CHARS` 経路を line 単位で golden 化できる。

#### 8.6.2 Glass 単一 tag `channel.glass` (P1-C 5a review)

Glass 側の AC-09 検証 (§7.2) は `adb logcat -s channel.glass:*` で **exact match
filter** をかける。`channel.glass-state` / `channel.glass.bridge` のような sub-tag
を導入すると verifier に拾われず invariant 検証が **silent に形骸化** する。

対策: `glass_state_swap` / `glass_input_swap` のような AC-09 target event は
**単一 tag `channel.glass`** に集約する (`TAG_GLASS_STATE` const で固定)。それ以外の
instance log (GlassBridge / ScreenAwake / NotificationSound 等) は `StructuredLog
(<sub>)` の sub-tag を使う (`channel.glass.bridge` 等)。「verifier が見るべき event
は単一 tag に必ず集約する」を契約とする。

#### 8.6.3 制御文字 regex の `\u` エスケープ統一 (P1-B 5a review)

旧コードは制御文字 regex を **raw 制御バイトの literal** で書こうとしたが、ファイル
経由で書き込むと表示変換されて ASCII printable に潰れる状態 (本来狙っていた
`\x00-\x1f, \x7f` レンジが抜けていた) になっていた。Write tool / IDE / VCS の
経路で raw bytes は壊れるので、**明示の `\u00XX` エスケープ** (`Regex("[\\u0000-\\u001F\\u007F]")`)
で書く規約に統一する (Phone 側と Glass 側どちらも同じ表現)。

#### 8.6.4 `phoneStateEmit` / `glassStateSwap` (AC-09 invariant target)

AC-09 検証ランナー (§7.2 `verify_atomicity.py`) が **target にする event name は
1 種類ずつ**:

| 側 | event name | tag |
|---|---|---|
| Phone | `phone_state_emit` | `channel.phone-state` |
| Glass | `glass_state_swap` | `channel.glass` |

両者とも **mode / pending_request_id / transcript_state / input_len / mic_source** を
1 行で出す契約 (§8.3 必須フィールド)。Phone 側は `confirming: Boolean` を呼び出し側が
渡す (CurrentState から復元できないため)。`InputTextOnly` 受信時も同 event 名
(`glass_state_swap`) で出して、verifier が複数 event を集約しなくて済むようにする
(P2-F 5a review)。
...
- session_id を新規生成せず、cwd 配下の既存 `*.jsonl` から interactive 選択した uuid を
  再使用
- `--resume $target` を渡す (claude の resume path)。`--session-id` は **併用しない**:
  claude CLI が `--session-id` と `--resume` の併用を `--fork-session` 無しでは
  reject する (`Error: --session-id can only be used with --continue or --resume if
  --fork-session is also specified.`) ため。resume 時の claude 側 session UUID は
  `--resume $target` だけで確定し、Bridge 側は MCP config `env.BRIDGE_SESSION_ID` で
  同じ uuid を別経路で受け取るので整合する
