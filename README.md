...
> [!NOTE]
> POC 記事 ([Qiita](https://qiita.com/)) では `claude-channel` という名前で紹介していますが、本リポは設計を一新した第 2 世代で `claude-mobile-hud` という名前に変えています。CLI subcommand の体系 (`hub` / `pair` / `run` / `resume`) は POC と互換。
...
CLI (`claude-mobile-hud`) は `$PATH` から実行できると POC と同じ使い心地になる。`~/.local/bin` を `$PATH` に入れている前提で symlink:
...
POC 記事と同じ流れ — **Hub 常駐 → 初回 pair → run** の 3 ステップ。
