...
> **Claude Code を PC から解放し、Phone と Glass で遠隔監視・承認・音声指示できる個人用モバイル操作環境。**

## 概要

Claude Code が autonomous (Auto Mode / `/goal`) になったぶん、ボトルネックは「AI の能力」より「人が PC の前にいるか」に寄ってきた。AI が走っているあいだの **確認・承認・指示出し** だけを Phone と Glass に逃がして、PC を離れても agent を止めない、というのが本プロジェクトの狙い。

- **送る** — Phone からテキスト / 音声 (OpenAI Realtime Whisper、確定前に手編集可能) で進行中の Claude セッションに指示
- **受ける** — Claude の reply が Phone 通知 + Rokid Glasses に同時配信、Rokid Glasses はスリープからも自動 wake → セッション画面に遷移
- **承認** — Bash 実行 / ファイル編集等の permission 要求が Phone 通知に「拒否 / 許可」ボタン付きで届く
- **並行セッション** — 複数 Claude セッションを同じ Phone から束ねて見れる、Glass で切替

## コンポーネント

- **phone** (Android): Hub に対する **単一クライアント**。テキスト / 音声 / 画像 / permission 応答 / セッション切替を担当し、必要なら Glass にも中継する
- **glass** (Rokid Glasses companion, 任意): Phone と CXR-L (Bluetooth ベースの制御プレーン) で接続。HUD にセッション一覧と reply / permission を表示、ジェスチャで Phone に指示を返す
- **hub** (PC、常駐デーモン): Phone から見た **単一の HTTP / SSE エンドポイント**。複数の Claude セッションを束ねる中継地点
- **bridge** (PC、1 セッション = 1 プロセス): Claude Code が `--mcp-config` 経由で spawn する **MCP サーバ**。Hub と IPC で繋がり、reply / permission を双方向に流す
...
進捗 (Phase 1–6) と Phase 5 完了後の持ち越しは [docs/TODO.md](./docs/TODO.md) を参照。
...
- Rokid Glasses + USB ペア済み Android phone (実機テスト時)
